...
export default class CurrencySetting extends Vue {
  ...
  currenType(index) {
    ...
    switch (index) {
      case "4123":
        this.currencyName = "游戏币";
        break;
      case "4125":
        this.currencyName = "魔石";
        break;
      case "4203":
        this.currencyName = "绑定魔石";
        break;
    }
    if (this.amount < 0) {
      let amount = Math.abs(this.amount)
      str = `成功减少${amount}个${this.currencyName}`;
    } else if (this.amount > 0) {
      str = `成功添加${this.amount}个${this.currencyName}`;
    } else{
      str = `未对货币进行改变`;
    }
    return str
  ...
  changeCurrency() {
    ...
    console.log(params);
    let str = this.currenType(this.currencyType);
    axios({
      method: "post",
      url: url,
      data: params
