<template>
  <div class="PlayerRoleInfo">
    ...
    <el-dialog title="角色属性" :visible.sync="roleAttr_Visible" width="20%">
      <div class="roleInfo_Dialog">
        <div>
          <span>战力：</span>
          <span>{{roleinfo.Power}}</span>
        </div>
        <div>
          <span>属性评分：</span>
          <span>{{roleinfo.AttributeScore}}</span>
        </div>
        <div>
          <span>战斗力构成数量：</span>
          <span>{{roleinfo.PowerList_count}}</span>
        </div>
        <div>
          <span>战斗力构成：</span>
          <el-table :data="roleinfo.PowerList" style="width: 100%">
...
@Component
// export default class PlayerAccountInfo extends Vue
export default class PlayerRoleInfo extends Vue {
  // data
  common = new Common();
  ...
  rolevalue(param1: any, param2: { RoleId: string; RoleName: string }) {
  ...
  searchRoleInfo() {
    // console.log(this.common.exchangeDate)
    if (this.openId === "") {
      this.$message({
        showClose: true,
        message: "请输入玩家账号",
  ...
  exchange(arr: any[]) {
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      arr[i].CreateTime = this.common.exchangeDate(arr[i].CreateTime);
      arr[i].LastLoginTime = this.common.exchangeDate(arr[i].LastLoginTime);
      arr[i].LastLogoutTime = this.common.exchangeDate(arr[i].LastLogoutTime); // 转日期
      if (arr[i].Job === 1) {
        arr[i].Job = "战士";
      } else if (arr[i].Job === 2) {
        arr[i].Job = "法师";
  ...
  // 角色VIP信息,军团信息,属性,名人堂
  handleSearch(index: any, row: { RoleId: any }, item: any) {
    let url;
    let params = {
      partition: Number(this.$store.state.serverId),
      openId: this.openId,
      ...
      .then(resp => {
        switch (item) {
          ...
          case 3:
            console.log(resp);
            if (resp.data.code === 200) {
              this.roleinfo = resp.data.msg;
              this.exchange(this.roleinfo.PowerList);
            }
            break;
          case 4:
