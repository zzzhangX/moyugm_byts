...
@Component
export default class HorseRaceLamp extends Vue {
  // data
  common = new Common();
  ...
  // methods
  lastTimeChange(value: number) {
    this.addLampForm.lastTime = value;
  }
  notifyCDChange(value: number) {
  ...
  exchange(arr: any) {
    ...
    for (let i = 0; i < arr.length; i++) {
      arr[i].StartTime = this.common.$exchangeDate(arr[i].StartTime); // 转日期
  ...
  noticeListSearch() {
    if (this.timeArr.length !== 2 || this.partition === "") {
      ...
      let params = {
        ...
        startTime: this.common.$dateToms(this.timeArr[0]),
        stopTime: this.common.$dateToms(this.timeArr[1]),
        pageNo: this.pageNo
      };
      console.log(params);
      axios({
  ...
  addLamp() {
    ...
    let params = {
      ...
      startTime: this.common.$dateToms(this.addLampForm.startTime),
  ...
  // 删除跑马灯
  handleDelete(index: any, row: { NoticeId: any; }) {
